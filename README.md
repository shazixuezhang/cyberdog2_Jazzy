...
libgz-sim-dev git python3-colcon-common-extensions
```
...
4. 编译失败缺少插件库
    检查系统`ros-jazzy-ros-gz`全套依赖是否完整安装；
5. git push推送提示non-fast-forward
    执行分支上游绑定：`git branch --set-upstream-to=cyberdog2_Jazzy main`后再pull/push。

## 开源协议
Apache License 2.0，基于小米CyberDog官方仿真源码二次修改，仅供学习研发使用。当前文件内容过长，豆包只阅读了前 82%。