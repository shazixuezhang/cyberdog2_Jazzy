...
#include "cyberdog_protocol/srv/bes_http_send_file.hpp"
...
public:
  explicit ReadyNotifyNode(rclcpp::Node::SharedPtr node_ptr)
  {
    ...
    self_check_status_pub_ = ready_notify_node_->create_publisher<cyberdog_protocol::msg::SelfCheckStatus>(
      "self_check_status",
      rclcpp::SystemDefaultsQoS(),
      pub_options
    );
    state_swith_status_pub_ =
      ready_notify_node_->create_publisher<cyberdog_protocol::msg::StateSwitchStatus>(
    ...
    motion_excute_client_ =
      ready_notify_node_->create_client<cyberdog_protocol::srv::MotionResultCmd>(
      "motion_result_cmd",
      rmw_qos_profile_services_default, ready_notify_callback_group_);

    audio_play_client_ =
      ready_notify_node_->create_client<cyberdog_protocol::srv::AudioTextPlay>(
      "speech_text_play",
      rmw_qos_profile_services_default, ready_notify_callback_group_);

    bms_status_sub_ =
      ready_notify_node_->create_subscription<cyberdog_protocol::msg::BmsStatus>(
      "bms_status", rclcpp::SystemDefaultsQoS(),
      std::bind(&ReadyNotifyNode::BmsStatus, this, std::placeholders::_1), sub_options);

    upload_events_client_ = ready_notify_node_->create_client<cyberdog_protocol::srv::BesHttpSendFile>(
      ...
      notify_selfcheck_thread = std::thread(
        [this, &stmap]() {
          ...
          while (!exit_ && rclcpp::ok()) {
            cyberdog_protocol::msg::SelfCheckStatus msg;
  ...
  bool UploadEvents()
  {
    ...
    auto req = std::make_shared<cyberdog_protocol::srv::BesHttpSendFile::Request>();
    req->method = cyberdog_protocol::srv::BesHttpSendFile::Request::HTTP_METHOD_POST;
  ...
  {
    cyberdog_protocol::msg::StateSwitchStatus sss;
    sss.code = 0;
    sss.state = state;
  ...
  void AppConnectState(const std_msgs::msg::Bool msg)
  {
    ...
    if (pre_connect_state == false && msg.data == true) {
      ...
      auto request_motion = std::make_shared<cyberdog_protocol::srv::MotionResultCmd::Request>();
  ...
  {
    auto request_audio = std::make_shared<cyberdog_protocol::srv::AudioTextPlay::Request>();
    request_audio->module_name = ready_notify_node_->get_name();
    request_audio->is_online = false;
    request_audio->speech.play_id = play_id;
    auto callback = [](rclcpp::Client<cyberdog_protocol::srv::AudioTextPlay>::SharedFuture future) {
...
private:
  ...
  rclcpp::Publisher<cyberdog_protocol::msg::SelfCheckStatus>::SharedPtr self_check_status_pub_;
  rclcpp::Publisher<cyberdog_protocol::msg::StateSwitchStatus>::SharedPtr state_swith_status_pub_;
  ...
  rclcpp::Client<cyberdog_protocol::srv::MotionResultCmd>::SharedPtr motion_excute_client_;
  rclcpp::Client<cyberdog_protocol::srv::AudioTextPlay>::SharedPtr audio_play_client_ {nullptr};
  rclcpp::Client<cyberdog_protocol::srv::BesHttpSendFile>::SharedPtr upload_events_client_ {nullptr};
