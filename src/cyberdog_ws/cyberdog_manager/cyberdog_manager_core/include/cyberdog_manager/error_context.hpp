...
#include "cyberdog_protocol/msg/node_error.hpp"
...
public:
  ...
  : name_(node_name), exit_(false)
  {
    ...
    node_error_sub_ =
      error_report_node_->create_subscription<cyberdog_protocol::msg::NodeError>(
...
private:
  void ErrorsCallback(const cyberdog_protocol::msg::NodeError::SharedPtr msg)
  {
    ErrorRecorder er;
    er.name = msg->name;
    er.code = msg->code;
...
private:
  ...
  bool exit_;
  std::thread thread_;
  rclcpp::Subscription<cyberdog_protocol::msg::NodeError>::SharedPtr node_error_sub_;
