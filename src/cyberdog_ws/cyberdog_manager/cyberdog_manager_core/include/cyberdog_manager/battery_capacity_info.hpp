...
#include "cyberdog_protocol/srv/bms_info.hpp"
...
class BatteryCapacityInfoNode final
{
  ...
  using BCINBMS_CALLBACK = std::function<void (const cyberdog_protocol::msg::BmsStatus::SharedPtr)>;
...
public:
  void Init()
  {
    ...
    bms_status_sub_ = battery_capacity_info_node_->create_subscription<cyberdog_protocol::msg::BmsStatus>(
      "bms_status", rclcpp::SystemDefaultsQoS(),
      std::bind(&BatteryCapacityInfoNode::BmsStatus, this, std::placeholders::_1),
      sub_options);
    bms_info_srv_ = battery_capacity_info_node_->create_service<cyberdog_protocol::srv::BmsInfo>(
      "bms_info", std::bind(
    ...
    audio_play_extend_pub =
      battery_capacity_info_node_->create_publisher<cyberdog_protocol::msg::AudioPlayExtend>(
      "speech_play_extend",
      rclcpp::SystemDefaultsQoS(), pub_options);
    audio_play_client_ = battery_capacity_info_node_->create_client<cyberdog_protocol::srv::AudioTextPlay>(
      "speech_text_play",
...
private:
    ...
    if (!bms_status_.power_wired_charging) {
      if (bms_status_.batt_soc <= 0) {
        if (!is_soc_zero_) {
          is_soc_zero_ = true;
          // is_soc_five_ = false;
          PlayAudioService(cyberdog_protocol::msg::AudioPlay::PID_PERCENT_0);
        }
      } else if (bms_status_.batt_soc < 5) {
        if (!is_soc_five_) {
          is_soc_five_ = true;
          is_soc_twenty_ = false;
          PlayAudio(cyberdog_protocol::msg::AudioPlay::PID_PERCENT_5);
        }
        ...
        if (!is_soc_twenty_) {
          ...
          PlayAudio(cyberdog_protocol::msg::AudioPlay::PID_PERCENT_20);
        }
      } else if (bms_status_.batt_soc < 30) {
        if (!is_soc_thirty_) {
          is_soc_thirty_ = true;
          is_soc_twenty_ = false;
          PlayAudio(cyberdog_protocol::msg::AudioPlay::PID_PERCENT_30);
        }
      }
    } else {
      is_soc_twenty_ = false;
  ...
  {
    cyberdog_protocol::msg::AudioPlayExtend msg;
    msg.is_online = false;
  ...
  void BmsInfo(
    const cyberdog_protocol::srv::BmsInfo::Request::SharedPtr,
    cyberdog_protocol::srv::BmsInfo::Response::SharedPtr response)
  {
...
private:
  ...
  rclcpp::Publisher<cyberdog_protocol::msg::AudioPlayExtend>::SharedPtr audio_play_extend_pub;
  rclcpp::Service<cyberdog_protocol::srv::BmsInfo>::SharedPtr bms_info_srv_;
