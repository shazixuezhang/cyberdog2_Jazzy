...
bool CtrlBluetooth::Init()
{
  ...
  try {
    this->bluetooth_status_sub_ =
      this->create_subscription<cyberdog_protocol::msg::BluetoothStatus>(
      "bluetooth_status", 1,
      std::bind(&CtrlBluetooth::BluetoothStatusCallback, this, std::placeholders::_1));

    this->control_advertise_client_ =
      this->create_client<cyberdog_protocol::srv::BmsCmd>("ctrl_advertising");
...
{
  std::shared_ptr<cyberdog_protocol::srv::BmsCmd::Request> request =
    std::make_shared<cyberdog_protocol::srv::BmsCmd::Request>();
  if (enable) {
