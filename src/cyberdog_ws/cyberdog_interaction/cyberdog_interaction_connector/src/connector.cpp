  ...
  const std::function<bool(bool)> _control_bluetooth)
{
  ...
  try {
    ...
    this->bluetooth_status_sub_ = this->create_subscription<cyberdog_protocol::msg::BluetoothStatus>(
      "bluetooth_status", rclcpp::SystemDefaultsQoS(),
      std::bind(&Connector::BtStatusCallback, this, std::placeholders::_1),
      sub_option);
    this->wifi_info_sub_ = this->create_subscription<cyberdog_protocol::msg::WifiInfo>(
      "app_send_network_data", rclcpp::SystemDefaultsQoS(),
      std::bind(&Connector::APPSendWiFiCallback, this, std::placeholders::_1),
      sub_option);
    this->notify_to_app_pub_ = this->create_publisher<cyberdog_protocol::msg::NotifyToApp>(
      "notify_to_app", 1, pub_option);
    this->bledfu_progress_sub_ = this->create_subscription<cyberdog_protocol::msg::BLEDFUProgress>(
...
void Connector::APPSendWiFiCallback(
  const cyberdog_protocol::msg::WifiInfo::SharedPtr msg)
{
...
void Connector::BtStatusCallback(
  const cyberdog_protocol::msg::BluetoothStatus::SharedPtr msg)
{
