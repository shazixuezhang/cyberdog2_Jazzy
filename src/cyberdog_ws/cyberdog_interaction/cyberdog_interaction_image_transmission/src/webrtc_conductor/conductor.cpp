...
: ros_node_(ros_node), video_source_(video_source)
{
  ...
  status_notify_client_ = ros_node_->create_client<cyberdog_protocol::srv::CameraService>("camera_service");
  ...
  bool activate, int height, int width, const std::string & alignment)
{
  ...
  request->command = activate ?
    cyberdog_protocol::srv::CameraService::Request::START_LIVE_STREAM :
    cyberdog_protocol::srv::CameraService::Request::STOP_LIVE_STREAM;
  if (activate) {
