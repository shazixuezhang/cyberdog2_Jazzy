...
public:
  ...
  : Node("uwb_tracking_action_client", options), is_running(false)
  {
    ...
    stop_algo_task_client_ =
      this->create_client<cyberdog_protocol::srv::StopAlgoTask>(
      "stop_algo_task",
      rmw_qos_profile_services_default, stop_callback_group_);
    current_connected_bluetooth_client_ =
      this->create_client<cyberdog_protocol::srv::BLEScan>(
      "get_connected_bluetooth_info",
      rmw_qos_profile_services_default, stop_callback_group_);
    tracking_status_sub_ = this->create_subscription<cyberdog_protocol::msg::AlgoTaskStatus>(
      "algo_task_status", 10,
      std::bind(&Uwb_Tracking_Client::get_tracing_status, this, std::placeholders::_1));
  }
  void get_tracing_status(const cyberdog_protocol::msg::AlgoTaskStatus::SharedPtr msg)
  {
  ...
  void cancel_goal()
  {
    if (is_uwb_tracking() || is_running.load()) {
      ...
      if (!stop_algo_task_client_->wait_for_service(std::chrono::seconds(2))) {
        ...
        auto req = std::make_shared<cyberdog_protocol::srv::StopAlgoTask::Request>();
        req->task_id = cyberdog_protocol::srv::StopAlgoTask::Request::ALGO_TASK_UWB_TRACKING;
...
private:
  ...
  rclcpp::Client<cyberdog_protocol::srv::StopAlgoTask>::SharedPtr stop_algo_task_client_;
  rclcpp::Client<cyberdog_protocol::srv::BLEScan>::SharedPtr current_connected_bluetooth_client_;
  rclcpp::Subscription<cyberdog_protocol::msg::AlgoTaskStatus>::SharedPtr tracking_status_sub_{nullptr};
