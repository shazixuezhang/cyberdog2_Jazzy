...
#include "cyberdog_protocol/srv/fs_machine.hpp"
...
{
  using FSMACHINE_SRV_T = cyberdog_protocol::srv::FsMachine;
...
private:
  ...
  void BuildClientMap()
  {
    ...
    for (size_t i = 0; i < target_vec_.size(); i++) {
      ...
      auto client = node_ptr_->create_client<cyberdog_protocol::srv::FsMachine>(client_name);
