...
bool cyberdog::device::DeviceHandler::Init(rclcpp::Node::SharedPtr node_ptr)
{
  ...
  touch_pub_ = node_ptr->create_publisher<cyberdog_protocol::msg::TouchStatus>("touch_status", 10);
  bms_pub_ = node_ptr->create_publisher<cyberdog_protocol::msg::BmsStatus>("bms_status", 10);
  uwb_pub_ = node_ptr->create_publisher<cyberdog_protocol::msg::UwbRaw>("uwb_raw", 10);
...
void cyberdog::device::DeviceHandler::ExecuteLed(
  const cyberdog_protocol::srv::LedExecute_Request::SharedPtr request,
  cyberdog_protocol::srv::LedExecute_Response::SharedPtr response)
{
  if (!led_inited_) {
...
void cyberdog::device::DeviceHandler::ExecuteBmsControl(
  const cyberdog_protocol::srv::BmsCmd_Request::SharedPtr request,
  cyberdog_protocol::srv::BmsCmd_Response::SharedPtr response)
{
  if (!bms_inited_) {
    response->code = (int32_t)system::KeyCode::kStateInvalid;
...
void cyberdog::device::DeviceHandler::ExecuteUwb(
  const cyberdog_protocol::srv::GetUWBMacSessionID_Request::SharedPtr request,
  cyberdog_protocol::srv::GetUWBMacSessionID_Response::SharedPtr response)
{
  if (!uwb_inited_) {
    response->code = (int32_t)system::KeyCode::kStateInvalid;
...
void cyberdog::device::DeviceHandler::PublishTouch(cyberdog_protocol::msg::TouchStatus msg)
{
...
void cyberdog::device::DeviceHandler::PublishBmsMessage(cyberdog_protocol::msg::BmsStatus msg)
{
  if (bms_pub_ != nullptr) {
...
void cyberdog::device::DeviceHandler::PublishUwbMessage(cyberdog_protocol::msg::UwbRaw msg)
{
  if (uwb_pub_ != nullptr) {
