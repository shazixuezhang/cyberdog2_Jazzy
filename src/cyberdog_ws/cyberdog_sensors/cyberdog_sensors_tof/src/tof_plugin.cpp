...
int32_t cyberdog::sensor::TofCarpo::Init(bool simulator)
{
  ...
  if (!simulator_) {
    ...
    for (auto & file : tof_cfg_files) {
      ...
      std::shared_ptr<cyberdog_protocol::msg::SingleTofPayload> tof_data =
        std::make_shared<cyberdog_protocol::msg::SingleTofPayload>();
...
void cyberdog::sensor::TofCarpo::SimulationThread()
{
  while (1) {
    ...
    if (is_working_) {
      ...
      const int datanum = cyberdog_protocol::msg::SingleTofPayload::TOF_DATA_NUM;
      std::vector<float> obj;
      for (size_t i = 0; i < datanum; i++) {
        obj.push_back(1.0f * cyberdog_protocol::msg::SingleTofPayload::SCALE_FACTOR);
      }
      auto tof_payload = std::make_shared<cyberdog_protocol::msg::SingleTofPayload>();
    ...
    if (label.name == "enable_on_ack") {
      ...
      for (size_t i = 0; i < datanum; i++) {
        obj_data.push_back(
          ...
          cyberdog_protocol::msg::SingleTofPayload::SCALE_FACTOR);
